...
Agent 执行时会返回 `status: launched`，并让 macOS Terminal 创建一个独立 shell 来运行同一份 `apply.command`。这个 shell 不依赖 WorkBuddy 进程，因此关闭 WorkBuddy 后仍会继续完成 CDP 启动、注入和校验；不再使用后台 `pending` 交接。
