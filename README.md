...
`apply` 会自动选择路径：已认证的皮肤会话会瞬时热切换；首次应用或会话已失效时才需要重启 WorkBuddy。重启前请先保存未完成的输入或任务。Ambient Skin 固定使用 CDP 端口 `9347`；端口被占用时会明确报错，不会自动漂移。

首次应用会返回 `handoff: true, status: pending`，表示独立重启进程已接管。它会精准结束 WorkBuddy 官方进程族，等待 2 秒，使用固定端口 `9347` 启动 CDP，然后注入并校验皮肤。不需要第二次确认。

如果通过支持 Skill 的 AI 操作，只需说“换成初音未来主题”。当需要重启时，AI 必须先让你选择执行方式，两种方式地位相同：

1. **Agent apply**：你确认一次重启，AI 执行 `apply`，等待完成后只校验一次。
2. **手动 Terminal**：AI 不操作 WorkBuddy，直接给出完整命令；你在本机终端执行，同步完成强制退出、CDP 启动、皮肤注入和验证。
...
```bash
"$HOME/.workbuddy/skills/workbuddy-ambient-skin/scripts/workbuddy-ambient.sh" terminal-apply --theme miku-neko-maid --restart confirmed
```
