...
Agent 执行时会返回 `status: launched`，并提交一个由 macOS `launchd` 托管的一次性任务来运行同一份 `apply.command`。任务不依赖 WorkBuddy 进程，关闭 WorkBuddy 后仍会继续完成 CDP 启动、注入和校验；不需要 Terminal 自动化权限。
